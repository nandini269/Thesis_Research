...
import numpy as np
from sklearn.preprocessing import scale, OneHotEncoder, Imputer, LabelEncoder
...
def pre_process(raw_data, categorical, impute=True, standardize=True, one_hot_encode=True):
    ...
    if impute:
        # if there is any categorical feature
        if np.array(categorical).any():
            ...
            le = LabelEncoder()
            raw_categorical_encoded = np.hstack([le.fit_transform(raw_categorical[:, j]).reshape(-1, 1) for j in range(raw_categorical.shape[1])])
            processed.append(imp_categorical.fit_transform(raw_categorical_encoded))

        # if there is any numeric feature
